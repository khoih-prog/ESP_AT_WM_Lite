...
// -- HTML page fragments
const char ESP_AT_HTML_HEAD_START[] /*PROGMEM*/ = "<!DOCTYPE html><html><head><title>SAM_DUE_AT_WM_Lite</title>";

const char ESP_AT_HTML_HEAD_STYLE[] /*PROGMEM*/ =
...
#define BIG_BUFFER_LEN        768
      ...
      totalDataSize = sizeof(ESP8266_AT_config) + totalLength + sizeof(readCheckSum);
...
#define BIG_BUFFER_LEN        768
      ...
      ESP_AT_LOGERROR5(F("CrCCsum=0x"), String(checkSum, HEX), F(",CrRCsum=0x"), String(readCheckSum, HEX),
    ...
    void dueFlashStorage_putDynamicData()
    {
      ...
      for (int i = 0; i < NUM_MENU_ITEMS; i++)
      {
        ...
        for (uint16_t j = 0; j < myMenuItems[i].maxlen; j++, _pointer++, /*offset++,*/ byteCount++)
        {
          ...
          {
            ESP_AT_LOGERROR1(F("Danger:dynamic data too long >"), sizeof(buffer));
          }
      ...
      ESP_AT_LOGERROR3(F("CrCCSum=0x"), String(checkSum, HEX), F(",byteCount="), byteCount);
    }
#endif
    ...
    void saveConfigData()
    {
      ...
      ESP_AT_LOGERROR1(F("SaveData,ChkSum=0x"), String(calChecksum, HEX));
    ...
    bool getConfigData()
    {
      ...
      ESP_AT_LOGWARN1(F("Simulate EEPROM, Sz:"), DATA_LENGTH);
...
#define MAX_NUM_WIFI_RECON_TRIES_PER_LOOP     1
    ...
    bool connectMultiWiFi(int retry_time)
    {
      ...
      if (lastConnectedIndex != 255)
      {
        ...
        {
          ESP_AT_LOGERROR3(F("Ignore invalid WiFi PW : index="), new_index, F(", PW="),
                           ESP8266_AT_config.WiFi_Creds[new_index].wifi_pw);
      ...
      if (server)
      {
        ...
        if (!id_Updated && (key == String("id")))
        {
          ...
          if (strlen(value.c_str()) < sizeof(ESP8266_AT_config.WiFi_Creds[0].wifi_ssid) - 1)
            ...
            strncpy(ESP8266_AT_config.WiFi_Creds[0].wifi_ssid, value.c_str(),
                    sizeof(ESP8266_AT_config.WiFi_Creds[0].wifi_ssid) - 1);
        }
        ...
        else if (!id1_Updated && (key == String("id1")))
        {
          ...
          if (strlen(value.c_str()) < sizeof(ESP8266_AT_config.WiFi_Creds[1].wifi_ssid) - 1)
            ...
            strncpy(ESP8266_AT_config.WiFi_Creds[1].wifi_ssid, value.c_str(),
                    sizeof(ESP8266_AT_config.WiFi_Creds[1].wifi_ssid) - 1);
        }
