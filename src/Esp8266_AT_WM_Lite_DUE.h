...
#define CONFIG_EEPROM_START    (EEPROM_START + DRD_FLAG_DATA_SIZE)
    ...
    bool dueFlashStorage_get()
    {
      ...
      ESP_AT_LOGDEBUG5(F("CrCCsum=0x"), String(checkSum, HEX), F(",CrRCsum=0x"), String(readCheckSum, HEX), 
    ...
    void dueFlashStorage_put()
    {
      ...
      {       
        ...
        for (uint16_t j = 0; j < myMenuItems[i].maxlen; j++, _pointer++, /*offset++,*/ byteCount++)
        {
          ...
          {
            ESP_AT_LOGDEBUG1(F("Danger:dynamic data too long >"), sizeof(buffer));
          }
      ...
      totalDataSize = sizeof(ESP8266_AT_config) + byteCount;
      
      ESP_AT_LOGDEBUG3(F("CrCCSum=0x"), String(checkSum, HEX), F(",byteCount="), byteCount);
    }
    ...
    void saveConfigData()
    {
      ...
      ESP_AT_LOGWARN1(F("SaveData,ChkSum=0x"), String(calChecksum, HEX));
    ...
    bool connectMultiWiFi(int retry_time)
    {
      ...
      ESP_AT_LOGWARN(F("ConMultiWifi"));
      
      if (static_IP != IPAddress(0, 0, 0, 0))
      {
        ESP_AT_LOGWARN(F("UseStatIP"));
      ...
      if (lastConnectedIndex != 255)
      {
        ...
        {    
          index = new_index;
          ESP_AT_LOGWARN3(F("Using index="), index, F(", lastConnectedIndex="), lastConnectedIndex);
        }
        else
        {
      ...
      else
      {
        ...
        if ( strlen(ESP8266_AT_config.WiFi_Creds[0].wifi_pw) >= PASSWORD_MIN_LEN )
        {    
          ESP_AT_LOGWARN(F("First connection, Using index=0"));
        }
        else
        {
      ...
      {         
        while ( 0 < retry_time )
        {      
          ESP_AT_LOGWARN1(F("Remaining retry_time="), retry_time);
          ...
          if (status == WL_CONNECTED)
          {
            ...
            ESP_AT_LOGWARN1(F("WOK, lastConnectedIndex="), lastConnectedIndex);
